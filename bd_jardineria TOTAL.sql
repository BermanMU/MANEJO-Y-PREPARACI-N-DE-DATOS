--dropping the database to start again
drop database if EXISTS db_jardineria;
GO
...
--first select to search data
--with this command all the info of the table "clientes" 
select * from cliente
go

--with this command all the info of the table "detalle_pedido" 
select * from detalle_pedido
go

--with this command all the info of the table "empleado" 
select * from empleado
go

--with this command all the info of the table "gama_producto" 
select * from gama_producto
go


--with this command all the info of the table "oficina" 
select * from oficina
go

--with this command all the info of the table "pago" 
select * from pago
go

--with this command all the info of the table "pedido" 
select * from pedido
go

--with this command all the info of the table "producto" 
select * from producto
go

-- Inserting data into all tables
-- Inserting data into table "oficina"
...
-- Inserting data into table "empleado"
...
-- Here an example of inserting data in table "gama_producto"
...
-- Inserting data into table "detalle_pedido"
...
--searching all without "representante de ventas"
...
select count (limite de credito)
...
--numero de empleado no me sale tampoco

select sum (numero_empleado)
as total_credito
from cliente
go
...
--sustituir lo del parentesis si queremos
...
select 
 ...
 sum (limite_credito) as maximo
from cliente
go

-- de tabla pedido fecha minima de pedido y maximo de pedido
select
...
---total de pedidos por cliente, solo los que tengan mas de 5 pedidos
--- PENDIENTE (ver imagen)
...
--gama de producto con precio de venta
...
--total pagado por los clientes, solo los que han pagado mucho
---Revisar Imagen

select sum(limite_credito) 
from cliente
where limite_credito > avg(limite_credito)
go
...
select codigo_oficina as oficina


--TOTAL PRODUCTOS vendidos por pedido (solo pedidos grandes


select * from pedido
go

select avg (cantidad) as promedio from detalle_pedido
go
...
--total de productos por pais (solo paises con muchos clientes
...
-- inner join pedido p on c.codigo_client= p.codigo_cliente
...
--Clientes que si tienen, pero que limite de credito (is not null)
select nombre_cliente, region
from cliente
...
--Reemplazar vaolres NULL con Coalesce
--Calesce sirve para mostrar otro valor cuando aparece NULL
...
---creacion de (Sintaxis) de Vistas ---
...
--Ejemplo2 : Vista de productos caros
...
---Ejemplo 4: Vista con funciones de agregacion group by

--total de productos por pais (solo paises con muchos clientes


create view producto_pais
as
select pais, 
 count (*)
 as total_clientes
